...
public class Main {
	...
	public static void main(String[] args) {
		...
		//Insertar un registro en una tabla de la bdatos
		  ...
		  //userPersist.setNombre("Eddy"); 
		  //userPersist.setApellido("Postillos");
		  //userPersist.setClave("XLCM"); 
		  //userPersist.setEstado(2); 
		  //userPersist.setFnacim(null);
		  //userPersist.setTipo(2);
		  //userPersist.setUsuario("edpos");
		  
		  //manager.getTransaction().begin();
		 // manager.persist(userPersist);
		  //manager.getTransaction().commit();
		  
		 //Usuario userFind = manager.find(Usuario.class, 2);
